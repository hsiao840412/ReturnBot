...
來源：`https://github.com/hsiao840412/ReturnBot/releases/latest/download/appcast.xml`。每次發布都必須保留同一簽章金鑰、bundle id `com.returnbot.app` 與 `RetuenBot.app` 名稱。私密金鑰在發布者的 macOS 鑰匙圈中，不在原始碼或 App 內；更换電腦須安全移轉金鑰，不可直接產生另一把。
